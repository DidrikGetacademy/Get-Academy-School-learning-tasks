...
let pikachu = {
  ...
  function ShowCaughtPokemonView(){
   ...
   for(let i = 0; i < player.Pokemon.length; i++){
    pokemonList += ` <div>name:${player.Pokemon[i].name}<br> level: ${player.Pokemon[i].level}<br> health:${player.Pokemon[i].health}</div> <img src="${player.Pokemon[i].image}" alt="${player.Pokemon[i].name}">`;
   }
    app.innerHTML = /*HTML*/`
